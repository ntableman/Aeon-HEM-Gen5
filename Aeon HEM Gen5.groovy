...
// metadata
metadata {
	definition (name: "Aeon HEM Gen5", namespace: "DuncanIdahoCT", author: "Dillon A. Miller") {
	...
	simulator {
		for (int i = 0; i <= 10000; i += 1000) {
	  	    status "power  ${i} W": 
			new hubitat.zwave.Zwave().meterV3.meterReport(scaledMeterValue: i, precision: 3, meterType: 1, scale: 2, size: 4).incomingMessage()
		}
		...
		if (encapsulatedCommand) {
			if (cmd.sourceEndPoint == 1) {
				if (encapsulatedCommand.scale == 2 ) {
						...
						sendEvent(name: "power1", value: dispValue as String, unit: "W", descriptionText: "L1 Power: ${formattedValue} Watts")
				}
				if (encapsulatedCommand.scale == 5 ) {
						...
						dispValue = "${formattedValue}"
						sendEvent(name: "current1", value: dispValue as String, unit: "A", descriptionText: "L1 Current: ${formattedValue} Amps")
				}
			}
				...
				if (encapsulatedCommand.scale == 5 ) {
						...
						formattedValue = String.format("%5.2f", newValue)
						dispValue = "${formattedValue}"
						sendEvent(name: "current2", value: dispValue as String, unit: "A", descriptionText: "L2 Current: ${formattedValue} Amps")
				}
			}
		}
	}
...
def configure() {
    ...
    //Parameter 2 [1 byte] = 0 //disable solar readings, in this case, the clamp direction does not matter
    //Parameter 3 [1 byte] = 0 //disable threshold reports
    //Parameter 101 [4 byte] = 1776399 //report all sensors
    //Parameter 111 [4 byte] = 240 //report all sensors every 240 seconds
	...
	//zwave.configurationV1.configurationSet(parameterNumber: 255, size: 1, scaledConfigurationValue: 1),
		...
		zwave.configurationV1.configurationSet(parameterNumber: 101, size: 4, scaledConfigurationValue: 1776399),
